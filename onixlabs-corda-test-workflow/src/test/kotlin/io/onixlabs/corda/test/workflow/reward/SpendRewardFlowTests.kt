...
class SpendRewardFlowTests : FlowTest() {
    ...
    override fun initialize() {
        Pipeline
            ...
            .run(nodeA) {
                val reward = it.tx.outRefsOfType<Reward>().single()
                SpendRewardFlow.Initiator(reward)
            }
            .finally { transaction = it }
    }

    @Test
    fun `SpendRewardFlow transaction should be signed by the initiator`() {
        transaction.verifyRequiredSignatures()
    }

    @Test
    fun `SpendRewardFlow should record a transaction for the owner and observers`() {

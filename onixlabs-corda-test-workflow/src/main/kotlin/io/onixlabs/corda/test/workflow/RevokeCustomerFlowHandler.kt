...
class RevokeCustomerFlowHandler(
    ...
    @InitiatedBy(RevokeCustomerFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {

        private companion object {
            object ObservingRevokedCustomerStep : ProgressTracker.Step("Observing revoked customer.") {
                override fun childProgressTracker() = tracker()
            }
        }

        override val progressTracker = ProgressTracker(ObservingRevokedCustomerStep)

        @Suspendable
        override fun call(): SignedTransaction {
            return subFlow(RevokeCustomerFlowHandler(session, ObservingRevokedCustomerStep.childProgressTracker()))
        }
    }
}

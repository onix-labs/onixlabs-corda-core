...
class SpendRewardFlowSignatureDataTests : FlowTest() {
    ...
    @Test
    fun `SpendRewardFlow cannot spend a reward when the issuer is the initiator`() {
        val exception = assertThrows<SignatureException> {
            Pipeline
                .create(network)
                .run(nodeA) {
                    IssueCustomerFlow.Initiator(CUSTOMER_1, observers = setOf(partyC))
                }
                .run(nodeC) {

...
class IssueRewardFlowTests : FlowTest() {
    ...
    override fun initialize() {
        Pipeline
            ...
            .finally {
                transaction = it
                reward = it.tx.outputsOfType<Reward>().single()
            }
    }

    @Test
    fun `IssueRewardFlow transaction should be signed by the initiator`() {
        transaction.verifyRequiredSignatures()
    }

    @Test
    fun `IssueRewardFlow should record a transaction for the owner and observers`() {
        listOf(nodeA, nodeC).forEach {
            it.transaction {
    ...
    @Test
    fun `IssueRewardFlow should record a reward for the owner and observers`() {
        listOf(nodeA, nodeC).forEach {
            it.transaction {
                val recordedTransaction = it.services.validatedTransactions.getTransaction(transaction.id)
                    ?: fail("Failed to find a recorded transaction with id: ${transaction.id}.")

                val recordedReward = recordedTransaction
                    .tx.outputsOfType<Reward>().singleOrNull()
                    ?: fail("Failed to find a recorded reward.")

                assertEquals(reward, recordedReward)
            }
        }
    }
}
